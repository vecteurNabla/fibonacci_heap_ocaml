...
(** [min f] returns the cell containing the minimum of [f]. *)
...
val extract_min : 'a fibHeap -> 'a * float
(** [extract_min f] returns the element with the lowest priority,
   along with its priority, and deletes it from f. *)

val iter : ('a fibTree Cdlist.cdlist) -> 'a fibHeap
(** [iter f h] applies [f] to each element of [h] *)
